...
Neither the running average $\bar{a}_n = \frac{1}{n}\sum_k a_k$ nor the total correlation $\TC_n$ is an ideal choice for a single scalar.
The running average converges to $a_\infty$ (the uninteresting residual floor), losing sensitivity to the learnable structure above it. $D_{Ca_n}$ inherits this problem, but it has demonstrated its utility experimentally.
