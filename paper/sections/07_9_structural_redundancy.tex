...
The matching yields a known ground truth.
Both generators sample words identically and, given the sampled words, assign them to same-class positions uniformly; they differ only in the order of the classes, which the canonical condition fixes to a single pattern and scrambling draws uniformly from all
\begin{equation}
\binom{\posPatternSlotsTotal}{\posPatternCountNouns,\,\posPatternCountVerbs,\,\posPatternCountPreps} = \frac{\posPatternSlotsTotal!}{\posPatternCountNouns!\,\posPatternCountVerbs!\,\posPatternCountPreps!} = \posPatternNumOrders
\label{eq:pos-pattern-orders}
\end{equation}
distinguishable orderings of the class multiset $\{\mathrm{N, N, N, V, P, P}\}$.
The scrambled generator's entropy therefore exceeds the canonical generator's by exactly $\log_2 \posPatternNumOrders \approx \posPatternOrderBits$ bits per sentence, about \posPatternOrderBitsPerByte{} bits per byte, so the scrambled sets are genuinely more diverse by a known amount.
