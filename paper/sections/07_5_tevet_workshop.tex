...
\paragraph{Headline result.} On the binary tasks $D_{Ca_n}$ is competitive with the strongest embedding baseline (Table~\ref{tab:contest}).
...
The headline takeaway is that an information-theoretic, embedding-free metric is competitive with a sentence-embedding baseline on a human-grounded eval, with no embedding model, no reference corpus, and no human labels in the metric itself.
