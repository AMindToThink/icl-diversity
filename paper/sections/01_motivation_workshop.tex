...
Creativity researchers routinely need to compare the diversity of outputs across generation processes: post-training stages of the same base model (does RLHF collapse modes?), decoding strategies (what does temperature trade off against?), prompting interventions (does a chain-of-thought prompt narrow the response distribution?), and human-AI hybrid pipelines (how does a model's contribution compare in spread to a human writer's drafts?).
Existing diversity metrics rely on embedding distances \citep{du2019boosting} or surface-level $n$-gram statistics, both of which conflate genuine content diversity with lexical or representational artefacts (Section~\ref{sec:related}).
...
We propose a metric, $D_{Ca_n} = C \times a_n$, that scores AI samples and human-written response sets through the same pipeline: per-byte log-probabilities of a base model $\theta$, in a \emph{single forward pass} over the concatenated responses (Figure~\ref{fig:pipeline}).
No embedding model, no reference corpus, no human labels, no auxiliary classifier --- only the per-token probabilities $\theta$ already produces.
...
The progressive conditional surprise curve $a_k$ (the per-byte cross-entropy of the $k$-th response given the previous $k{-}1$) captures this signal directly, and its last point $a_n$ estimates the asymptotic floor (Section~\ref{sec:progressive-conditioning}).
...
The product $D_{Ca_n} = C \times a_n$ is the score; it is plausibility-weighted residual diversity in bits per byte.
We validate it against Tevet and Berant's human-grounded diversity benchmark (Section~\ref{sec:tevet}) and on the OLMo-2-7B post-training pipeline (Section~\ref{sec:rlhf-experiment}), and release a working open-source implementation alongside.\footnote{Code link withheld for double-blind review; see anonymous mirror in supplementary materials.}
The metric measures diversity as $\theta$ perceives it: outputs differing only in ways $\theta$ cannot distinguish in context appear less diverse, and outputs in which $\theta$ hallucinates structure appear less diverse than they are.
This relativity tightens automatically as base models improve.
