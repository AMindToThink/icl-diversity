...
Machine learning researchers studying creativity
...
We propose a metric, ``Decan'' or $D_{Ca_n} = C \times a_n$, that scores AI samples and human-written response sets through the same pipeline: per-byte log-probabilities of a base model $\theta$, in a \emph{single forward pass} over the concatenated responses (Figure~\ref{fig:pipeline}).
