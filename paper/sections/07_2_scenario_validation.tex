...
We report several candidate scalars: unconditional surprise $a_1$, the floor $a_n$, coherence $C$, our recommended score $D_{Ca_n} = C \times a_n$ (\textbf{bold}), the excess entropy $E$ (Appendix~\ref{app:excess-entropy}), $C \times E$, and the coherence spread $\sigma_\ell$ (the standard deviation of the per-byte cross-entropies $\{h_\theta(r_i \mid p)\}_{i=1}^n$). $D_{Ca_n}$ correctly ranks multi-mode above one-mode and suppresses incoherent scenarios via $C$.
