...
\end{equation}
the reciprocal of the geometric-mean per-byte perplexity that $\theta$ assigns to the responses individually.
...
The five edge-case behaviors are summarized in Table~\ref{tab:edge-cases}.

\begin{table}[t]
\centering
\small
\caption{Edge-case behavior of $D_{Ca_n} = C \times a_n$.
The product correctly suppresses pure noise and incoherent multi-mode (via $C$) and correctly suppresses single-mode and paraphrase sets (via $a_n$).}
\label{tab:edge-cases}
\begin{tabular}{@{}lccc@{}}
\toprule
Scenario & $C$ & $a_n$ & $D_{Ca_n}$ \\
\midrule
Pure noise                   & $\approx 0$ & high   & $\approx 0$ \\
Multi-mode, incoherent       & low         & high   & low (via $C$) \\
Multi-mode, coherent         & high        & high   & \textbf{high} \\
One coherent mode            & high        & low    & $\approx 0$ \\
Coherent paraphrases         & high        & low    & low (via $a_n$) \\
\bottomrule
\end{tabular}
\end{table}
