...
To test whether a stronger base model improves the metric on Tevet's benchmarks, we re-ran the full evaluation using Qwen3-30B-A3B-Base (a 30B-parameter mixture-of-experts model with $\sim$3B active parameters per token) via the Tinker API,\footnote{\url{https://thinkingmachines.ai/tinker}} with the same setup as in Section~\ref{sec:tevet}: completion-format prompting, 50 permutations, no fine-tuning.
