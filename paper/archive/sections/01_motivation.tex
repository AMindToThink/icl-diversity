...
If $\pi$'s responses are repetitive or constrained to a narrow set of modes, then conditioning on one response should significantly reduce $\theta$'s surprise at subsequent responses, leaving a low floor at $a_n$.
...
All quantities derived below (the progressive conditional surprise curve, its floor, and the coherence term) measure properties of $\pi$'s outputs \emph{as perceived by $\theta$}.
...
\begin{itemize}[leftmargin=*, itemsep=2pt]
  ...
  \item We introduce a separate \emph{coherence} term $C$ measuring per-byte plausibility of outputs under $\theta$, and combine it with the curve value $a_n$ to yield a single plausibility-weighted diversity score $\mathbf{D_{Ca_n} = C \times a_n}$ (Sections~\ref{sec:coherence} and \ref{sec:scalar}).
