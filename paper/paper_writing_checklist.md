...
- [ ] Datasets and model weights are released (e.g., via Hugging Face) where applicable. *(Base models used are all public; evaluation outputs and scenario response sets are in the repo. Verify before submission whether any custom-generated data needs a dedicated HF release.)*
...
- [ ] A dedicated **Limitations** section exists and is honest — it documents constraints we know about, not just generic disclaimers. *(No standalone \section{Limitations}. Limitations are instead scattered across the intro caveat paragraph, §sec:exp-discussion, and the C×E limitations subsection in the appendix. Consider consolidating into a short dedicated section with pointers back to the appendices — the content is there but hard for a reviewer to find.)* — `\paragraph{Important caveat.}`; `sec:exp-discussion`
...
- [x] `uv run python scripts/verify_cites.py --tex paper/in_context_diversity_metric.tex` passes for \cite{} resolution; soft-failure for 8 unused entries in `refs.bib` (`dror2017replicability`, `dror2018hitchhiker`, `dubois2024alpacafarm`, `kirk2023rlhf`, `olmo2024olmo2`, `padmakumar2023writingreduces`, `reimers2019sbert`, `zhang2025verbalizedsampling`) — candidates for cleanup but not failures.
