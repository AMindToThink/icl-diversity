...
A separate coherence term lets $D_{Ca_n}$ suppress these cases.
...
$D_{Ca_n}$ works in both the many-draws regime (where modes repeat and $a_\infty$ reflects the irreducible floor after $\theta$ has learned all recurring patterns) and the few-draws regime (where each response is unique and $a_\infty$ captures how much surprise survives full conditioning).

The edge cases behave correctly:
\begin{enumerate}[nosep]
    \item \textbf{Pure noise}: $C \approx 0$ (incoherent), so $D_{Ca_n} \approx 0$.
    \item \textbf{Multiple incoherent patterns}: $C$ low, $a_\infty$ high, but $D_{Ca_n}$ is suppressed by $C$.
    \item \textbf{Rich coherent diversity}: $C$ high, $a_\infty$ high, so $D_{Ca_n}$ is high.
    \item \textbf{One coherent pattern}: $C$ high, $a_\infty$ low ($\theta$ learns the mode), so $D_{Ca_n} \approx 0$.
    \item \textbf{Non-diverse paraphrases}: $C$ high, $a_\infty$ low ($\theta$ recognizes shared meaning), so $D_{Ca_n}$ is low.
\end{enumerate}
...
This is exactly the behavior $D_{Ca_n} = C \times a_n$ needs: the asymptotic floor tracks mode count directly.
...
We found it empirically inferior to $D_{Ca_n}$ on external benchmarks.
...
\begin{equation}
    \bar{D}_{a_\infty}(\pi) = \frac{1}{M}\sum_{j=1}^{M} D_{Ca_n}(\pi, p_j)
\end{equation}

To compare two policies $\pi_1$ and $\pi_2$, the natural scalar is the difference:
\begin{equation}
    \Delta D_{Ca_n} = \bar{D}_{a_\infty}(\pi_1) - \bar{D}_{a_\infty}(\pi_2)
\end{equation}
...
Applying $\Delta D_{Ca_n}$ to compare, e.g., a base model against an RLHF-trained variant across a prompt suite is a natural next step.
