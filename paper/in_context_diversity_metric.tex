...
\paragraph{Relationship to total correlation.} The excess entropy and total correlation are complementary decompositions of the same underlying structure. Working in expectation over i.i.d.\ draws from $\pi$, let $\bar{h} = \E[h_\theta(r \mid p)]$ denote the expected unconditional cross-entropy (identical for all $k$), $\bar{a}_k = \E[a_k]$ the expected conditional cross-entropy at position $k$, and $e_k = \bar{a}_k - a_\infty$ the excess at step $k$.
...
\begin{equation}
    I_k = (\bar{h} - a_\infty) - e_k
\end{equation}

The total correlation is the sum of per-step MI:
\begin{equation}
    \TC_n = \sum_{k=1}^{n} I_k = n(\bar{h} - a_\infty) - \sum_{k=1}^{n} e_k = n(\bar{h} - a_\infty) - E_n
\end{equation}

Equivalently:
\begin{equation}\label{eq:tc-decomp}
    E_n = n(\bar{h} - a_\infty) - \TC_n
\end{equation}

For large $n$ where $E_n \to E$, the total correlation grows linearly with slope $(\bar{h} - a_\infty)$:
\begin{equation}
    \TC_n \approx n(\bar{h} - a_\infty) - E
\end{equation}

The three quantities have clean interpretations: $(\bar{h} - a_\infty)$ is the per-response redundancy once $\theta$ has fully learned the available structure (the asymptotic MI rate); $E$ is the total finite structural information, consumed during $\theta$'s transient learning phase; and $\TC_n$ is the cumulative redundancy, which grows without bound because even after all learnable structure has been extracted, each new response is partially predictable.

Figure~\ref{fig:decomposition} illustrates this decomposition. At each step $k$, the gap between $a_\infty$ and $\bar{h}$ decomposes into $I_k$ (how much $\theta$ has learned by step $k$) and $e_k$ (how much remains to be learned). The excess entropy $E$ is the total area of the blue region; the total correlation $\TC_n$ is the total area of the red region.
...
\addplot[name path=hbar, forget plot, draw=none]
    coordinates {(0.5,1.2) (12.5,1.2)};

% Define a_infinity line
\addplot[name path=ainf, forget plot, draw=none]
    coordinates {(0.5,0.4) (12.5,0.4)};

% Define a_k curve (theoretical, in expectation)
\addplot[name path=ak, forget plot, draw=none]
    coordinates {
        (0.5,1.2) (1,1.18) (2,1.15) (3,1.10) (4,1.02)
        (5,0.88) (6,0.70) (7,0.55) (8,0.46) (9,0.42)
        (10,0.41) (11,0.40) (12,0.40) (12.5,0.40)
    };
...
\addplot[thick, dashed, black]
    coordinates {(0.5,1.2) (12.5,1.2)};
\addlegendentry{$\bar{h}$ (unconditional)}
...
\addplot[thick, black, mark=*, mark size=1.5pt]
    coordinates {
        (1,1.18) (2,1.15) (3,1.10) (4,1.02)
        (5,0.88) (6,0.70) (7,0.55) (8,0.46) (9,0.42)
        (10,0.41) (11,0.40) (12,0.40)
    };
\addlegendentry{$\bar{a}_k$ curve}

% Labels for the colored regions
\node[font=\small] at (axis cs:10,0.95) {$\TC_n = \sum_k I_k$};
\node[font=\small] at (axis cs:4,0.60) {$E = \sum_k e_k$};

% Annotation arrows showing I_k and e_k for a single step (k=6, a_6=0.70)
\draw[<->, thick, gray] (axis cs:6,0.70) -- node[right, font=\footnotesize] {$e_k$} (axis cs:6,0.40);
\draw[<->, thick, gray] (axis cs:6,1.2) -- node[right, font=\footnotesize] {$I_k$} (axis cs:6,0.70);

\end{axis}
\end{tikzpicture}
\caption{Decomposition of the gap between unconditional surprise $\bar{h}$ and the asymptotic floor $a_\infty$ at each step $k$. The per-step gap $\bar{h} - a_\infty$ splits into mutual information $I_k = \bar{h} - \bar{a}_k$ (\textcolor{red!70}{red}, above the curve) and excess $e_k = \bar{a}_k - a_\infty$ (\textcolor{blue!70}{blue}, below the curve). Summing across $k$: the red area is the total correlation $\TC_n$; the blue area is the excess entropy $E$. As $k$ grows, $e_k \to 0$ and each step contributes the full $\bar{h} - a_\infty$ to $\TC_n$, so $\TC_n$ grows linearly while $E$ converges.}
\label{fig:decomposition}
\end{figure}

\subsection{Byte-Weighted Excess Entropy and Total Structural Information}

The definition $E = \sum_k (a_k - a_\infty)$ above sums per-byte rates across responses without accounting for response length. The per-byte normalization is essential for the $a_k$ curve itself: if response 3 is 200 bytes and response 7 is 50 bytes, their raw total log-probabilities are not comparable as points on the same plot. The per-byte rate makes each $a_k$ a property of ``how surprising is this response per unit of text,'' which is what lets the curve reveal structure visually and what makes the total correlation decomposition work (each $e_k$ is a per-byte rate in the same units).

However, as a \emph{scalar summary}, the unweighted sum $\sum_k e_k$ has a flaw: a long response carrying slightly excess surprise contributes the same as a short one, despite containing more total information. We therefore define two related quantities that properly account for response length, while retaining the per-byte $a_k$ curve as the primary diagnostic.

The \textbf{total structural information}:
\begin{equation}\label{eq:total-E}
    \mathcal{E} = \sum_{k=1}^{n} \|r_k\| \cdot (a_k - a_\infty)
\end{equation}

Units: bits. This is the total learnable structure that $\theta$ extracts from seeing $\pi$'s responses in sequence, above the residual floor.\footnote{One might worry that multiplying a shared $a_\infty$ by $\|r_k\|$ is incorrect, since different responses might have different residual floors. By the i.i.d.\ assumption, $a_\infty$ is the asymptotic per-byte conditional cross-entropy for \emph{any} response placed at a late position, regardless of its length or content --- exchangeability ensures the residual rate does not depend on which response ends up at position $k$. For a specific sample, the ``true floor'' for $r_k$ may differ from $a_\infty$, but this error is zero-mean across samples and is no worse than the same approximation already made in the unweighted $E = \sum_k (a_k - a_\infty)$. The dominant source of estimation error remains the approximation $a_\infty \approx a_n$.} Each response contributes its excess rate times its byte count. In practice, we estimate $a_\infty \approx a_n$ as before: $\hat{\mathcal{E}}_n = \sum_k \|r_k\| \cdot (a_k - a_n)$.

The \textbf{byte-weighted excess entropy rate}:
\begin{equation}\label{eq:weighted-E}
    E = \frac{\mathcal{E}}{\sum_{k=1}^{n}\|r_k\|} = \frac{\sum_k \|r_k\| \cdot (a_k - a_\infty)}{\sum_k \|r_k\|}
\end{equation}

Units: bits/byte. This is the length-weighted average excess rate, where each response's contribution is proportional to its byte count. If all responses have the same length, this reduces to the unweighted average. The two quantities are related by $\mathcal{E} = E \times \sum_k \|r_k\|$.
...
\paragraph{Total coherence.} For the non-length-normalized diversity score (Section~\ref{sec:scalar}), we also define the \textbf{total coherence}:
\begin{equation}\label{eq:total-coherence}
    \mathcal{C} = 2^{-\frac{1}{n}\sum_{i=1}^{n} \|r_i\| \cdot h_\theta(r_i \mid p)}
\end{equation}

This is the geometric mean of the total probability $\theta(r_i \mid p)$ across responses, living in $(0,1)$.\footnote{Since $\|r_i\| \cdot h_\theta(r_i \mid p) = -\log_2 \theta(r_i \mid p)$, we have $\mathcal{C} = \left(\prod_i \theta(r_i \mid p)\right)^{1/n}$.} Total probability is a property of the string, not of the tokenization: by the chain rule, $\theta(r \mid p) = \prod_t \theta(r^t \mid r^{<t}, p)$ regardless of how $\theta$ factors the sequence. Consequently, $\mathcal{C}$ is comparable across base models with different tokenizers (though not across models of different quality, as with $C$).
...
First, it breaks the information-theoretic interpretation. $E = \sum_k e_k$ has a clean meaning as the total structural information above the noise floor, connected to the excess entropy of Crutchfield and Feldman \citeyearpar{crutchfield2003regularities}. Inserting weights makes $E_w$ a hybrid quantity that is not the excess entropy of any well-defined process. In particular, the idealized relationship $E = \log_2 m / \bar{B}$ for equally-weighted modes relies on the unweighted sum, and inserting weights breaks this equality.
...
\begin{itemize}[nosep]
    \item $E$ (bits/byte): ``How much learnable structure does $\theta$ find among $\pi$'s outputs, per byte?''
...
Units: bits/byte. This is coherence-weighted structural information per byte: ``how much learnable structure is there per byte, discounted by how plausible the outputs are?'' Because both $C$ and $E$ are length-normalized, $D$ is comparable across prompts that elicit responses of different lengths.

The \textbf{total diversity score}:
\begin{equation}\label{eq:D-total}
    \mathcal{D} = \mathcal{C} \times \mathcal{E}
\end{equation}
...
The two scores are complementary. $D$ is the right choice when comparing policies that may produce responses of different lengths on the same prompt, or when averaging across a prompt suite (Section~\ref{sec:aggregation}). $\mathcal{D}$ is the right choice when the total amount of diverse information matters regardless of response length.
...
The edge cases behave correctly for both $D$ and $\mathcal{D}$:
\begin{enumerate}[nosep]
    \item \textbf{Pure noise}: $C \approx 0$, $E \approx 0$, so $D \approx 0$ and $\mathcal{D} \approx 0$.
    ...
    \item \textbf{Decomposable}: One can always factor $D$ back into $C$ and $E$ (or $\mathcal{D}$ into $\mathcal{C}$ and $\mathcal{E}$) to identify which term drove a change.
\end{enumerate}
...
The width of this band, $D_+ - D_- = C(2^{\sigma_\ell} - 2^{-\sigma_\ell}) \times E$, is zero when all responses have equal coherence and large when the policy mixes coherent and incoherent modes. A wide band is the diagnostic signal for heterogeneous output quality.
...
While $E$ measures the total structural information (in bits/byte) that $\theta$ can learn from $\pi$'s outputs, it does not directly yield a mode count. The na\"ive formula $m_{\mathrm{eff}} = 2^{\bar{B} \cdot E}$ assumes that all learnable structure corresponds to mode identity, which holds only for near-deterministic modes. For real LLM outputs, $\theta$ learns far more from context than which mode is active --- it learns shared stylistic patterns, vocabulary, topic-specific structure, and other within-mode regularities. This inflates $E$ well beyond $\log_2 m / \bar{B}$, making $m_{\mathrm{eff}}$ unreliable.
...
\paragraph{Total scores.} The total diversity score $\mathcal{D}$ can be aggregated in the same way: $\bar{\mathcal{D}} = \frac{1}{M}\sum_j \mathcal{D}(\pi, p_j)$ and $\Delta\mathcal{D} = \bar{\mathcal{D}}_{\mathrm{base}} - \bar{\mathcal{D}}_{\mathrm{steered}}$, with $\Delta\mathcal{D}$ in bits. The choice between $D$ and $\mathcal{D}$ for aggregation depends on whether length variation across prompts should be normalized away (use $D$) or counted as part of the diversity signal (use $\mathcal{D}$).
...
\begin{enumerate}[nosep]
    ...
    \item $D = C \times E$ (bits/byte) when a single per-prompt number is needed and $\sigma_\ell$ is small. $\bar{D}$ averaged across a prompt suite for a single-policy summary. Alternatively, $\mathcal{D} = \mathcal{C} \times \mathcal{E}$ (bits) when total information matters more than per-byte rate.
    \item $\Delta D = \bar{D}_{\mathrm{base}} - \bar{D}_{\mathrm{steered}}$ (or $\Delta\mathcal{D}$) when comparing two policies across a prompt suite.
\end{enumerate}
...
The forward pass produces the log-probability of every token conditioned on all preceding tokens. The tokens belonging to $r_k$ are conditioned on exactly $p, r_1, \ldots, r_{k-1}$ (plus formatting), which is the conditioning required for $a_k$. Partitioning the output log-probabilities by response boundary and normalizing by byte count yields all $n$ values of $a_k$ simultaneously. The cost is a single forward pass over the full concatenation, with FLOPs scaling as $O((|p| + n\bar{L}_{\mathrm{tok}})^2)$ from causal attention, where $\bar{L}_{\mathrm{tok}}$ is the average response length in tokens.
...
The $a_k$ values depend on the ordering of $\{r_i\}$. In expectation over random permutations the summary statistics ($E$, $D$) are invariant, but for a single evaluation run there is ordering noise. Averaging over a small number of random permutations (e.g., 3--5) reduces this variance at a proportional increase in cost.
...
\paragraph{Fallback for incompatible tokenizers.} When the desired models use different tokenizers, one can average the per-byte $a_k$ values across models, since bits/byte is a common unit regardless of tokenization. This is not a principled probabilistic mixture and does not provide the robustness guarantee of token-level ensembling, but it may still reduce variance from individual model quirks when the models are not simultaneously out of distribution.
...
\paragraph{Perplexity.} The coherence term $C = 1/\mathrm{PPL}$ (Section~\ref{sec:coherence}) connects this framework to the standard language modeling evaluation metric. The primary scalar $D = C \times E$ operates at the per-byte level, avoiding any unit conversion to response-level quantities.

\paragraph{Total correlation.} The total correlation $\TC_n$ relates to the excess entropy via Eq.~\eqref{eq:tc-decomp}: $\TC_n = n(\bar{h} - a_\infty) - E_n$. See Section~\ref{sec:excess-entropy} and Figure~\ref{fig:decomposition} for the full derivation and a visual decomposition. Note that since all quantities are cross-entropies under $\theta$ rather than true entropies of $\pi$'s output distribution, $\TC_n$ is a $\theta$-relative quantity that measures the total correlation \emph{as perceived by} $\theta$; it equals the true total correlation only to the extent that $\theta$ approximates $\pi$'s joint distribution.
