...
Their product $D_{a_\infty} = C \times a_\infty$ yields a single scalar: plausibility-weighted residual diversity in bits per byte.
...
\begin{itemize}[leftmargin=*, itemsep=2pt]
  ...
  \item We introduce a separate \emph{coherence} term $C$ measuring per-byte plausibility of outputs under $\theta$, and combine it with the asymptotic curve value $a_\infty$ to yield a single plausibility-weighted diversity score $\mathbf{D_{a_\infty} = C \times a_\infty}$ (Sections~\ref{sec:coherence} and \ref{sec:scalar}).
...
We adopt $D_{a_\infty}$ because it is interpretable and works empirically across our validation regimes, not because it is uniquely correct.
...
Full numbers and reproduction commands are in \texttt{investigations/length\_matched\_rlhf/VERDICT.md}.
...
\section{Related Concepts}\label{sec:related}
