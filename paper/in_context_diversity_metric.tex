...
Their product $D_{a_\infty} = C \times a_\infty$ yields a single scalar: plausibility-weighted residual diversity in bits per byte, that does not reward noise and works in both the many-draws regime (repeated modes) and the few-draws regime (unique responses).
