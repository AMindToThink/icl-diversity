...
The \textcolor{blue!70!black}{\textbf{unconditional track}} (right) scores each $r_i$ independently against the prompt alone, again per-byte, and the geometric mean defines coherence $C$.
The product $D_{a_\infty} = C \times a_n$ measures plausibility-weighted residual diversity (bits/byte).
...
We use ``bits'' to match standard usage in the information-theory literature.} This is the total surprise of the response, a property of the string itself, independent of $\theta$'s tokenizer (since the chain rule yields the same total regardless of how the sequence is factored).
...
\end{equation}
Units: bits/byte.
Normalizing by byte count rather than token count makes this rate independent of $\theta$'s tokenizer, enabling comparisons across base models with different vocabularies.
...
This motivates decomposing the metric into separate coherence and independence components.
...
Unless otherwise noted, all reported $a_k$ values and the diversity score $D_{a_\infty}$ are in per-byte units.
...
The noise policy correctly gets a low $D_{a_\infty}$ because of the coherence term, not because of the curve alone.

The problem the curve alone cannot solve is a \textbf{policy that has multiple distinct but individually incoherent modes}.
Consider a badly trained or modified model that produces several recognizably different types of low-quality output.
The base model $\theta$ can learn from context which type is being produced (so $a_k$ drops meaningfully across $k$ and $a_\infty$ may be low), but each type is still implausible text.
Without a separate plausibility term, the policy could score as meaningfully diverse.
A separate coherence term lets $D_{a_\infty}$ suppress these cases.
...
Equivalently, $C$ is the reciprocal of the geometric-mean per-byte perplexity:
\begin{equation}
...
Section~\ref{sec:tevet} compares $D_{a_\infty}$ against alternative scalars (raw $a_n$, $C$ alone, the excess-entropy-based $C \times E$) and finds that $D_{a_\infty}$ outperforms all of them on external benchmarks.
...
\paragraph{Typical range of $C$.} Published bits-per-byte values for English text range from $\sim$0.72 (GPT-3 davinci on The Pile \citep{gao2020pile}) to $\sim$1.05 (GPT-2 XL), corresponding to $C \in [0.49, 0.61]$.
In our Tevet evaluation with Qwen2.5-3B, we observe per-sample bits-per-byte values ranging from 0.5 to 2.3, giving $C \in [0.20, 0.70]$, with a mean around 0.4--0.5 for coherent text.
...
When comparing policies, overlaying curves on the same axes provides immediate visual insight.
    \item The summary triple $(a_n, C, \sigma_\ell)$ together with the scalar $D_{a_\infty} = C \times a_n$ (bits/byte).
When $\sigma_\ell$ is small, $D_{a_\infty}$ is a reliable scalar; when $\sigma_\ell$ is large, report the diversity band $[D_-, D_+]$ and examine which modes are incoherent via the per-response $c_i = 2^{-h_\theta(r_i \mid p)}$ values.
\end{enumerate}

The score is defined per prompt $p$.
When comparing policies across a prompt suite, one can average $D_{a_\infty}$ over prompts or take differences; see Appendix~\ref{app:aggregation} for details.
...
We report several candidate scalars: unconditional surprise $a_1$, the asymptotic floor $a_n$, coherence $C$, our recommended score $D_{a_\infty} = C \times a_n$ (\textbf{bold}), the excess entropy $E$ (Appendix~\ref{app:excess-entropy}), $C \times E$, and the coherence spread $\sigma_\ell$. $D_{a_\infty}$ correctly ranks multi-mode above one-mode and suppresses incoherent scenarios via $C$.
...
\caption{Progressive conditional surprise curves ($a_k$ in total bits) for all five scenarios, comparing GPT-2 (left) and Qwen2.5-3B (right).
Shaded regions show $\pm 1$ standard deviation across 100 permutations.
...
The floor $a_n$ increases monotonically with $m$ (\modeCountAnMone{} bits at $m=1$ to \modeCountAnMten{} bits at $m=10$; see Table~\ref{tab:mode-count}), reflecting more irreducible entropy remaining after full conditioning.
This is exactly the behavior $D_{a_\infty} = C \times a_n$ needs: the asymptotic floor tracks mode count directly.
...
For binary datasets we report ROC AUC (the natural evaluation for binary classification); for DecTest we report Spearman $\rho$ with temperature.

\paragraph{Caveats.} The benchmark provides only 5 responses per set (McDiv\_nuggets, ConTest), well below the 20+ responses needed for $E$ (Appendix~\ref{app:excess-entropy}) to converge.
Additionally, McDiv\_nuggets has a dataset construction confound: low-diversity sets are paraphrases of specific, dramatic endings that are intrinsically more surprising to the base model than the generic completions in high-diversity sets (see Appendix~\ref{app:confound}).
...
\paragraph{Alternative scalars.} The $C$ weighting consistently helps: raw $a_n$ OCA is \tevetAnVsCxAnOCAMinPct\%--\tevetAnVsCxAnOCAMaxPct\% lower than $C \times a_n$ OCA across the nine binary tasks.
...
The $C$ weighting in $D_{a_\infty}$ thus helps rather than hurts on this benchmark, but for reasons tied to this dataset's construction; we expect $a_n$ to carry most of the signal in settings without such a confound.
...
The coherence term $C$ and spread $\sigma_\ell$ are permutation-invariant by construction and are unaffected.
...
The sigmoid fit still provides a useful parametric form for extrapolating $a_\infty$, even when the inflection point is degenerate ($k_0 < 1$).
...
\paragraph{The framework admits other metrics.} The $a_k$ curve is the primary object of our framework, and $D_{a_\infty} = C \times a_n$ is one natural summary of it among many.
...
We have not explored this space systematically; we focus on $D_{a_\infty}$ because it empirically works well across regimes and has a clean information-theoretic interpretation, but we expect the framework to support a family of related metrics with complementary properties.
...
\paragraph{Explicit framing of multi-response contexts.} We observe a characteristic increase in conditional surprise from $k=1$ to $k=2$ in many settings: after seeing the first response, $\theta$ appears to expect the second to continue on the same theme, so a genuinely alternative response is extra-surprising.
Explicit framing---e.g., prefixing the context with ``the following are several different completions of the same prompt''---could shift this prior and reduce the $k=2$ bump, improving the metric's sensitivity in the few-draws regime.
...
We found it empirically inferior to $D_{a_\infty}$ on external benchmarks.

\paragraph{Perplexity.} The coherence term $C = 1/\mathrm{PPL}$ (Section~\ref{sec:coherence}) connects this framework to the standard language modeling evaluation metric.
The primary diversity score $D_{a_\infty} = C \times a_\infty$ operates in bits/byte, weighting the residual surprise floor by output plausibility.
...
Our metric leverages a related insight: the base model's ability to compress responses in context provides a meaningful signal about their diversity structure.
...
\paragraph{Sampling diversity metrics.} Work on decoding-time diversity \citep{holtzman2020curious} typically operates at the surface level ($n$-gram overlap, self-BLEU).
Our metric operates at the distributional level and can distinguish policies that produce lexically varied but semantically redundant outputs.
...
The $a_k$ framework provides a text-level alternative: collapse should appear as a drop in $a_\infty$ on the agent's own outputs, independent of reward structure.

\paragraph{Diversity evaluation benchmarks.}
\citet{tevet2020evaluating} introduced the first systematic framework for evaluating diversity metrics, with their McDiv benchmark providing human-labeled response sets at two diversity levels.
We use McDiv for validation (Appendix~\ref{app:confound}), though we identify data quality issues including label contamination and a construction confound.
...
This appendix develops the excess entropy $E$, an alternative summary of the $a_k$ curve that is theoretically interesting (connecting to the excess entropy of computational mechanics \citep{crutchfield2003regularities} and to the total correlation) but empirically inferior to $D_{a_\infty}$ on the external benchmarks of Section~\ref{sec:tevet}.
We include it here for theoretical completeness and to explain the empirical comparisons that motivate our preference for $D_{a_\infty}$.
...
These limitations are the reason $D_{a_\infty}$ is preferred as the primary score: $a_\infty$ is the \emph{level} of the floor, which differs meaningfully between diverse and non-diverse response sets even when the curve stays nearly flat.
...
This is the fundamental reason $D_{a_\infty}$ is preferred.
...
The diversity score $D_{a_\infty}$ is defined relative to a single prompt $p$.
...
$\Delta D_{a_\infty}$ answers ``how much more plausibility-weighted residual diversity does $\pi_1$ have than $\pi_2$, on average across prompts?''
No division is involved, so $\Delta D_{a_\infty}$ is stable even when either policy's score is near zero.

\paragraph{Why not a ratio?} The relative measure $D(\pi_2) / D(\pi_1)$ is appealing (``what fraction of diversity survived?'') but requires division by $D(\pi_1)$, which is near zero for any constrained prompt.
Clamping the denominator introduces a hyperparameter.
The absolute difference avoids these issues.

\paragraph{Limitation.} We have not yet validated the aggregation machinery by sampling from actual models and comparing their diversity scores across prompts.
Our experiments evaluate diversity of fixed response sets (human-written or synthetically constructed).
Applying $\Delta D_{a_\infty}$ to compare, e.g., a base model against an RLHF-trained variant across a prompt suite is a natural next step.
