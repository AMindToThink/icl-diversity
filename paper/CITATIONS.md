...
- **Preserve citation keys by default.** The old hand-written keys (e.g. `lam2025noveltybench`) survive into the new system even when the actual first author is different (Zhang, in that case). Renaming keys churns every `\cite{}` site and is deferrable. `natbib`/`plainnat` renders the author from the bib entry's `author=` field, not the key, so wrong keys with right metadata produce right output.
