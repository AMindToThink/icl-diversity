...
The generating stage owns roughly nine-tenths of the variance at either level
on both prompt sets; grader identity is a single-digit-percent factor and the
grader × stage interaction is smaller still. Restricting to the in-family 4×4
submatrix (dropping the cross-family Qwen row) leaves the split essentially
unchanged, so the small grader share is not a family-switch artifact. Exact
