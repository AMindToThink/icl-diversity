...
def wilcoxon_greater(x: np.ndarray, y: np.ndarray, name: str) -> dict[str, Any]:
...
def direction_check(x: np.ndarray, y: np.ndarray, name: str) -> dict[str, Any]:
...
def kendall_tau_check(data: dict[str, Any], scenario: str) -> dict[str, Any]:
    ...
    for m in data["scenarios"][scenario]:
        ...
        taus.append(
            {"tau": float(tau), "p": float(p), "label": m.get("prompt_label", "?")}
        )
...
def run_cross_model_tests(
    ...
    for scenario in [
        ...
        mono_detail[scenario] = {
            "gpt2": g_n,
            "qwen": q_n,
            "total": len(qwen["scenarios"][scenario]),
        }
    results.append(
        {
...
def print_results(
    ...
    for r in q13_results:
        ...
        if "U" in r:
            ...
            print(
                f"        x_mean={r['x_mean']:.4f}, y_mean={r['y_mean']:.4f}, U={r['U']:.1f}, p={r['p']:.4f}"
            )
        elif "taus" in r:
            print(f"  {status:4s}  {name}")
            for t in r["taus"]:
    ...
    for r in cross_results:
        ...
        if "W" in r:
            ...
            print(f"  {status:4s}  {name}")
            print(
                f"        q_mean={r['x_mean']:.4f}, g_mean={r['y_mean']:.4f}, +:{r['n_pos']} -:{r['n_neg']}, {w_str}, p={r['p']:.4f}"
            )
        elif "threshold" in r:
            print(f"  {status:4s}  {name}")
            print(f"        mean={r['mean']:.4f}, threshold={r['threshold']}")
            ...
            print(
                f"        q_mean={r.get('q_mean', r.get('x_mean', 0)):.4f}, g_mean={r.get('g_mean', r.get('y_mean', 0)):.4f}, ratio={r['ratio']:.2f}x"
            )
        elif "detail" in r:
            print(f"  {status:4s}  {name}")
            print(
                f"        GPT-2 total: {r['gpt2_total']}/25, Qwen total: {r['qwen_total']}/25"
            )
            for s, d in r["detail"].items():
                print(
                    f"          {s}: GPT-2={d['gpt2']}/{d['total']}, Qwen={d['qwen']}/{d['total']}"
                )
        elif "closer_to_zero" in r:
            print(f"  {status:4s}  {name}")
            print(
                f"        E_q={r['q_mean']:.4f}, E_g={r['g_mean']:.4f}, Qwen closer to 0: {r['closer_to_zero']}"
            )
        elif "x_mean" in r:
            print(f"  {status:4s}  {name}")
