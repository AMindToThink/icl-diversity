...
def plot_metric_correlation(
    ...
    icl_map = {
        (r["stage"], r["prompt_id"]): r.get("diversity_score_D")
        for r in icl_rows
        if r.get("prompt_set") == prompt_set
    }
    base_map_ead = {
...
def write_results_table(analysis: dict, out_path: Path) -> None:
    ...
    for pset in ["alpacaeval", "nbcurated"]:
        a = analysis[pset]
        ms = a["stage_means"]["diversity_score_D"]
        tests = a["tests"]["diversity_score_D"]
...
def analyze(icl_rows: list[dict], baseline_rows: list[dict]) -> dict:
    ...
    for pset in ["alpacaeval", "nbcurated"]:
        per_stage = {
            "diversity_score_D": collect_metric(icl_rows, "diversity_score_D", pset),
            "a_n": collect_metric(icl_rows, "a_n", pset),
            "C": collect_metric(icl_rows, "C", pset),
...
def main() -> None:
    ...
    for pset in ["alpacaeval", "nbcurated"]:
        ...
        per_stage_D = collect_metric(icl, "diversity_score_D", pset)
