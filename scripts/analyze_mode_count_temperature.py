...
def group_D_by_m(data: dict[str, Any]) -> dict[int, list[float]]:
    """Extract D values grouped by mode count m."""
...
def h1_variance_reduction(
    ...
    ref_by_m = group_D_by_m(all_results[1.0])
    ...
    for temp in temperatures:
        by_m = group_D_by_m(all_results[temp])
...
def h2_ordering_preservation(
    ...
    ref_by_m = group_D_by_m(all_results[1.0])
    mode_counts = sorted(ref_by_m.keys())
    ref_means = [float(np.mean(ref_by_m[m])) for m in mode_counts]

    # Plot: mean D vs m at each temperature
    fig, ax = plt.subplots(figsize=(8, 5))
    for temp in temperatures:
        by_m = group_D_by_m(all_results[temp])
        means = [float(np.mean(by_m[m])) for m in mode_counts]
        sems = [float(np.std(by_m[m]) / np.sqrt(len(by_m[m]))) for m in mode_counts]
        ax.errorbar(mode_counts, means, yerr=sems, fmt="o-", label=f"T={temp}",
                     markersize=4, capsize=2)

    ax.set_xlabel("Number of modes m")
    ax.set_ylabel("mean D (diversity score)")
    ...
    print("\n  H2' Spearman rho of mean_D(m) at T vs T=1.0 (10 mode counts)")
