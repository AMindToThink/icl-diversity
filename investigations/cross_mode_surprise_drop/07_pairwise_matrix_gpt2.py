...
DTYPE = torch.float32
MODEL_NAME = "gpt2"
OUTPUT_DIR = Path(__file__).parent / "figures" / "gpt2"
...
# ---------------------------------------------------------------------------
# Compute unconditional surprises (N x M forward passes)
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# Compute conditional surprises (N x N x M forward passes)
...
print(f"Computing {N_MODES}x{N_MODES}x{M_SAMPLES} = {total_passes} conditional surprises...")
...
print("=== GPT-2 Surprise Reduction Matrix (bits) ===")
...
ax.set_title(f"GPT-2: Mean Surprise Reduction (bits, n={M_SAMPLES})\npositive = context helps")
...
ax2.set_title(f"GPT-2: Std of Surprise Reduction (bits, n={M_SAMPLES})")
...
ax3.set_title(f"GPT-2: Unconditional surprise per mode (n={M_SAMPLES})")
ax3.invert_yaxis()
...
ax.plot(fit_x, fit_y, color="red", linewidth=1, linestyle="--", alpha=0.7,
        label=f"y = {coeffs[0]:.2f}x {coeffs[1]:+.1f}  (R^2={np.corrcoef(row_means, col_means)[0,1]**2:.2f})")
...
ax.set_title("GPT-2: Mode informativeness vs. context benefit")
ax.set_aspect("equal")
plt.tight_layout()
