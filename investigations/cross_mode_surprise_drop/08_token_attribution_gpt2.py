...
GPT-2 version of 08_token_attribution.py.
...
def main() -> None:
    ...
    fig.suptitle("GPT-2: Token-Level Surprise Attribution", fontsize=14, fontweight="bold", y=1.01)
    ...
    for idx, (res, ax) in enumerate(zip(results, axes)):
        ...
        ax.set_title(
            f"GPT-2: {res['context_mode']} -> {res['target_mode']}"
            + (" (same)" if res["is_same_mode"] else "")
            + f"    total delta = {res['total_delta_bits']:+.1f} bits",
            fontsize=10,
            fontweight="bold",
        )
    ...
    ax.set_title("GPT-2: Where Is Cross-Mode Surprise Reduction Concentrated?")
    ax.legend()
