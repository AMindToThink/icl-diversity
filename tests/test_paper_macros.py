...
def test_paper_macros_all_resolve() -> None:
    ...
    # \tevetInv* macros share the "tevet" prefix but are generated by
    # scripts/analyze_tevet_grader_invariance.py into their own file
    # (referenced by the grader-robustness appendix, appG).
    tevet_inv = (
        PROJECT_ROOT / "results" / "tevet" / "grader_invariance" / "tevet_invariance_macros.tex"
    )
    if tevet_inv.exists():
        defined |= set(re.findall(r"\\newcommand\{\\([A-Za-z]+)\}", tevet_inv.read_text()))
