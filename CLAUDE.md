...
- **BPE-merged boundary tokens go to the separator.** When a tokenizer emits a single token spanning a response and the following separator (e.g., Qwen's `.\n\n`), `_find_response_boundaries` attributes the merged token to the separator, not the response. The response's trailing character contributes to its byte-count denominator but not to its cross-entropy numerator. Regression test: `tests/test_response_boundaries.py::TestBoundaryRoundtrip::test_no_separator_leaks_into_response`.
